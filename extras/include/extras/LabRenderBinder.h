...
struct lab_texture     { private: friend struct lab_render_mgr; int handle{}; };
...
// meshes
LRB_API lab_mesh lab_render_mesh_load(const char* path);
...
LRB_API lab_mesh lab_render_mesh_rustum(float znear, float zfar, float yfov, float aspect);

// GL interface... because it has to interoperate with other systems.
